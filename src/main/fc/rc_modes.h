...
typedef enum { MODELOGIC_OR = 0, MODELOGIC_AND } modeLogic_e;

// type to hold enough bits for CHECKBOX_ITEM_COUNT. Struct used for value-like
// behavior
...
#define CHANNEL_VALUE_TO_STEP(channelValue)                                    \
  ((constrain(channelValue, CHANNEL_RANGE_MIN, CHANNEL_RANGE_MAX) -            \
    CHANNEL_RANGE_MIN) /                                                       \
   25)
...
PG_DECLARE_ARRAY(modeActivationCondition_t, MAX_MODE_ACTIVATION_CONDITION_COUNT,
...
STATIC_ASSERT(BOXUSER4 + 1 - BOXUSER1 == BOX_USER_NAME_COUNT,
...
bool isModeActivationConditionConfigured(
